	...
	for i in range(194, 806, 51):
		...
		if strike_through_striker:
			for coin in strike_through_striker:
				...
				angle_striker_coin_pocket = ang(coin_pocket, coin_striker)
				distance = distance_between_points((coin_x, coin_y), pocket3_point)
	...
	for i in range(806, 194, -51):
		...
		if strike_through_striker:
			for coin in strike_through_striker:
				...
				coin_striker = ((coin_x, coin_y),(striker_x, striker_y))
				angle_striker_coin_pocket = ang(coin_pocket, coin_striker)
				distance = distance_between_points((coin_x, coin_y), pocket1_point)
				if angle_striker_coin_pocket < 150:
					if distance > 200:
						continue
				coin_point = (coin_x, coin_y)
...
def pocket2_reverse(args):
	...
	for coin in positions:
		...
		for j in args:
			...
			if j == coin:
				continue
			if Point(pos_x, pos_y).intersects(LineString((intersection_point, coin_point)).buffer(50)) or Point(pos_x, pos_y).intersects(LineString((coin_point, strike_point, pocket2_point)).buffer(50)):
				path = False
				break
		if path:
			strike['angle'] = angle
