...
def coin_positions(*args):
	...
	print '\n------------MANUAL?????------------\n'
	inp, o, e = select.select( [sys.stdin], [], [], 0)
	if inp:
		white_x = []
		white_y = []
		black_x = []
...
def coin_positions4(args, return_dict):
	...
	for coin in args:
		coin_x = coin['x']
		coin_y = coin['y']
		if coin_y < 194 + 55 or coin_x < 153 + 55:
			continue
		else:
...
def coin_positions2(args, return_dict):
	...
	for coin in args:
		coin_x = coin['x']
		coin_y = coin['y']
		if coin_y > 806 - 55 or coin_x < 153 + 55:
			continue
		else:
			positions.append(coin)
...
# 		coin_x = coin['x']
# 		coin_y = coin['y']
# 		if coin_y < 244 or coin_x > 153 + 50:
# 			continue
# 		else:
# 			positions.append(coin)
...
# 		coin_x = coin['x']
# 		coin_y = coin['y']
# 		if coin_y > 756 or coin_x > 153 + 50:
# 			continue
# 		else:
# 			positions.append(coin)
